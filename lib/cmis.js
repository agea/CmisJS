...
//----
    ...
    cmis.getObject = function (objectId, options) {
        ...
        return _get(cmis.repo.rootFolderUrl).query(options).execute();
    };
    ...
    cmis.getObjectByPath = function (path, options) {
        ...
        return _get(cmis.repo.rootFolderUrl + path).query(options).execute();
    };


    cmis.createFolder = function (parentId, input, policies, addACEs, removeACEs){
        var options = _fill({});
        ...
        options.repositoryId = cmis.repo.repositoryId;
        options.cmisaction = 'createFolder';
        return _post(cmis.repo.rootFolderUrl).send(options).execute();
    }

    cmis.deleteObject = function (objectId, allVersions){
        var options = _fill({});
        options.repositoryId = cmis.repo.repositoryId;
    ...
    // `ok` sets callback when response status == 2XX
    //
    ...
    // `notOk` sets callback when response status != 2XX
    //
    //      @param callback {Function}
    //      @return request {Request}
    ...
    request.Request.prototype.execute = function () {
        var r = this;
